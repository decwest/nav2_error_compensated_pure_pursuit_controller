...
TEST(EcppTerms, GateOffMatchesPurePursuit)
{
  ...
  const auto terms = ecpp_math::computeEcppTerms(0.7, 0.1, 0.1, 0.5, 0.5, 0.5, params);
...
TEST(EcppTerms, AlwaysOnAppliesCompensation)
{
  ...
  const auto terms = ecpp_math::computeEcppTerms(0.0, 0.1, 0.1, 0.5, 0.5, 0.5, params);
...
{
  const auto aligned = ecpp_math::computeEcppTerms(
    0.0, 0.0, 0.0, 0.5, 0.5, 0.5, defaultParams(GateMode::EY_ONLY));
  const auto perpendicular = ecpp_math::computeEcppTerms(
    0.0, 0.0, M_PI / 2.0, 0.5, 0.5, 0.5, defaultParams(GateMode::EY_ONLY));
  EXPECT_DOUBLE_EQ(perpendicular.sigma, aligned.sigma);
  EXPECT_GT(perpendicular.sigma, 0.98);
  EXPECT_DOUBLE_EQ(perpendicular.sigma_psi, 1.0);
}
...
TEST(EcppTerms, EyOnlyGateClosesFarFromPath)
{
  ...
  const auto terms = ecpp_math::computeEcppTerms(
    0.0, 1.0, 0.0, 0.5, 0.5, 0.5, defaultParams(GateMode::EY_ONLY));
...
TEST(EcppTerms, NominalLookaheadSetsPurePursuitGainAndLateralGateError)
{
  ...
  // At the design speed v = 0.95, K_y = (0.95/0.95)^2 = 1. The PP gain uses
  // the nominal lookahead: Delta K_y = 1 - 2 / 2^2 = 0.5.
  const auto terms = ecpp_math::computeEcppTerms(
    0.4, e_y, 0.1, nominal_lookahead_dist, 0.95, 0.95, params);
...
TEST(EcppTerms, DesignGainsMatchAtConfiguredSpeed)
{
  for (const double desired_speed : {0.2, 0.5, 0.9}) {
    for (const double epsilon : {0.01, 0.05, 0.1}) {
      auto params = defaultParams(GateMode::ALWAYS_ON);
      params.omega_n = 1.030;
      params.zeta = 0.707;
      params.v_epsilon = epsilon;
      const auto terms = ecpp_math::computeEcppTerms(
        0.4, 0.2, 0.1, 1.0, desired_speed, desired_speed, params);
      EXPECT_NEAR(terms.dK_y + 2.0, std::pow(1.030 / desired_speed, 2), 1e-12);
      EXPECT_NEAR(terms.dK_psi + 2.0, 2.0 * 0.707 * 1.030 / desired_speed, 1e-12);
    }
  }
}

TEST(EcppTerms, AdditiveRegularizationRemainsFiniteAtRestAndUsesSpeedMagnitude)
{
  const auto params = defaultParams(GateMode::ALWAYS_ON);
  const auto stopped = ecpp_math::computeEcppTerms(0.3, 0.2, 0.4, 1.0, 0.0, 0.5, params);
  EXPECT_TRUE(std::isfinite(stopped.curvature));
  EXPECT_DOUBLE_EQ(stopped.v_gain, 0.05);
  // omega_n = 2 at v0 = 0.5 gives configured omega_n = 2.2.
  for (const double speed : {0.0, 0.01, 0.2, 0.5, 1.0}) {
    const auto forward = ecpp_math::computeEcppTerms(0.3, 0.2, 0.4, 1.0, speed, 0.5, params);
    const auto reverse = ecpp_math::computeEcppTerms(0.3, 0.2, 0.4, 1.0, -speed, 0.5, params);
    EXPECT_NEAR(forward.dK_y + 2.0, std::pow(2.2 / (speed + 0.05), 2), 1e-9);
    EXPECT_NEAR(forward.dK_psi + 2.0, 4.4 / (speed + 0.05), 1e-12);
    EXPECT_DOUBLE_EQ(forward.curvature, reverse.curvature);
  }
}

TEST(EcppGoldenVector, MatchesDesignGainAndSharedProjectionDefinition)
{
  ...
  const auto terms = ecpp_math::computeEcppTerms(
    kappa_pp, path_error.e_y, path_error.e_psi, kLookahead, 0.5, 0.5, params);

  const double expected_v_gain = 0.55;
  const double expected_k_y = std::pow(1.030 / 0.5, 2.0);
  const double expected_k_psi = 2.0 * 1.030 / 0.5;
...
TEST(EcppGoldenVector, MatchesNonzeroHeadingAndEyOnlyGate)
{
  ...
  const double expected_v_gain = 0.55;
  const double expected_dk_y = std::pow(1.030 / 0.5, 2.0) - 8.0;
  const double expected_dk_psi = 2.0 * 1.030 / 0.5 - 4.0;
...
TEST(PathFrameError, StraightOffsetPath)
{
  ...
  const auto err = ecpp_math::computePathFrameError(arc_lookahead::projectPath(path));
...
{
  EXPECT_FALSE(ecpp_math::computePathFrameError(arc_lookahead::projectPath(makePath({}))).valid);
  EXPECT_FALSE(
    ecpp_math::computePathFrameError(arc_lookahead::projectPath(makePath({{0.0, 0.0}}))).valid);
  // Duplicate points only -> no usable segment.
  EXPECT_FALSE(
    ecpp_math::computePathFrameError(
      arc_lookahead::projectPath(makePath({{0.1, 0.0}, {0.1, 0.0}}))).valid);
}
